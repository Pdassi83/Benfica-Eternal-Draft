...
export default function Home(){
 ...
 return <main>
  ...
  {view==="home"&&<>
   <section className="hero"><div className="hero-copy"><img className="cosme" src={COSME_DAMIAO} alt="Cosme Damião em 1917"/><h1>Onze lendas.<br/><i>Uma Europa eterna.</i></h1><p>Constrói o melhor Benfica de sempre e enfrenta as equipas que marcaram a Taça dos Campeões e a Champions League.</p><a className="primary" href="#setup">Começar o draft →</a><div className="facts"><span><b>100</b>Lendas</span><span><b>10</b>Treinadores</span><span><b>{rivals.length}</b>Campeões</span></div><button className="archive-link" onClick={()=>setArchiveOpen(true)}>Explorar as 100 lendas ↗</button></div><div className="hero-art"><span className="stamp">1904 — ∞</span><div className="crest-evolution"><figure><img src={FIRST_CREST} alt="Primeiro emblema do Grupo Sport Lisboa"/><figcaption>Sport Lisboa · 1904</figcaption></figure><span><i>origem</i><b>×</b><i>eterno</i></span><figure><img src={CURRENT_CREST} alt="Emblema atual do Sport Lisboa e Benfica"/><figcaption>Sport Lisboa e Benfica · atual</figcaption></figure></div></div></section>
   <section className="setup" id="setup"><div className="title"><span>Configuração</span><div><h2>Prepara o teu Benfica</h2><p>A formação define as onze rondas do draft.</p></div></div><div className="setup-grid"><article><h3>Formação</h3><div className="formations">{(Object.keys(formations)as Formation[]).map(f=><button className={formation===f?"selected":""} onClick={()=>setFormation(f)} key={f}><b>{f}</b><small>{formations[f].join(" · ")}</small><i>{formation===f?"✓":"+"}</i></button>)}</div><Pitch formation={formation} picks={[]}/></article><article><h3>Treinador</h3><div className="coaches">{coaches.map(c=><button className={coachId===c.id?"selected":""} onClick={()=>setCoachId(c.id)} key={c.id}><b>{c.rating}</b><span><strong>{c.name}</strong><small>{c.era} · {c.style}</small></span><i>{coachId===c.id?"✓":"+"}</i></button>)}</div><p className="bonus"><span>Vantagem</span><b>{coach.bonus}</b></p></article></div><button className="primary wide" onClick={start}>Entrar na sala do draft →</button></section>
   <section className="identity-preview"><div><span className="eyebrow">Coleção Eternal · 100/100</span><h2>Cem identidades.<br/><i>Nenhum rosto inventado.</i></h2><p>A escolha depende do legado, da posição e dos atributos — nunca de uma imagem pouco fiel.</p></div></section>
